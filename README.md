# 知枝（Study Thread）

**知枝**是一款 **AI 伴读桌面应用**：在学习会话中与 AI 对话，划选摘录沉淀原子笔记，通过间隔重复按时复习——把每一次问答、笔记与复习，沉淀成可检索、可回顾的学习资产。

> 当前版本：**v0.3.1 测试版（Beta）** · Windows 10 1809+（内置 WebView2）
>
> 学习数据完全**本地优先**：全部存于你自己的 Markdown Vault；官方模型通道为**可选**，凭据只存系统钥匙串。

---

## 为什么是知枝？

传统学习工具的痛点：看过就忘、笔记散落各处、复习靠自觉、计划难坚持。知枝把「规划 → 学 → 摘 → 练」串成一个闭环：

1. **规划**：把学习目标告诉 AI 规划师，访谈式确认现状与可用时间，生成阶段化的每日任务；
2. **学**：与 AI 对话，把不懂的概念问清楚；
3. **摘**：在回答上划选关键内容，一键摘录成笔记；
4. **练**：笔记到期后自动进入复习队列，按记忆规律间隔重复，把短时记忆变成长期记忆。

每一步的产物都是**纯本地 Markdown 文件**，可检索、可导出、可备份，随时回顾你走过的学习路径。

---

## 与众不同之处

- **划选即笔记**：不是问完再手动复制粘贴——直接在 AI 回答上划选摘录，保留原始上下文，复习时随时回到出处；
- **来源可回溯**：回答中引用你的笔记与资料处带 `[n]` 角标，点击即见命中片段与 PDF 页码，可跳转原文——AI 说的每句话都有出处；
- **伴读教练（可选）**：引导模式先了解你的基础，小步讲解、设检查点、引导你产出，而不是直接丢一屏答案；
- **会话是树，不是流水账**：任何一条线索都能分叉成新分支深挖追问，不打断主线，思考路径一目了然；
- **复习不靠背卡片**：AI 按笔记内容自动出题（填空、问答、判断、情景题等），FSRS 间隔重复调度——检验的是理解，不是背诵原文；
- **计划由 AI 规划师定制**：几轮问答生成「阶段 + 每日任务」的学习计划书，任务可直接「开始学习」，学完自动关联会话、更新进度；
- **数据可携带**：Obsidian 兼容的纯 Markdown Vault，复制文件夹即备份；内置离线向量检索（RAG），断网也能召回自己的知识库；
- **模型自由**：官方通道登录即用（免配 Key），也可接入任意主流服务商或本地 Ollama，随时一键切换。

---

## 核心功能

### 学习会话

与 AI 进行多轮对话式学习，支持主流模型服务商。会话以 Markdown 存储在 Vault 中，可随时回看、继续。

- 支持 **知枝官方通道**（登录即用，免配 Key）与 **Anthropic (Claude)、OpenAI、DeepSeek、通义千问、智谱 (GLM)、Ollama（本地）** 及自定义服务商，一键切换；
- 内置本地向量检索：参考资料 + 历史笔记可在对话中召回，回答更贴合你的知识库；
- **来源锚定**：引用来源处带 `[n]` 角标，点击可预览出处（含 PDF 页码）并跳转原文；
- **引导模式**：需要真正学会时开启——AI 先了解你的基础，小步讲解并引导你思考；
- 支持上传 PDF / Markdown / 图片作为参考资料，供对话检索引用。

### 学习计划（AI 规划师）

「30 天入门 Rust」「三个月备考」——把目标交给规划师，通过几轮问答确认基础与每日可用时间，生成一份「目标 → 阶段 → 每日任务」的学习计划书。

- 计划以 Markdown 存放在 `<Vault>/plans/`，随手可改，也可让 AI 重新生成；
- 学习地图中查看今日任务，点击「开始学习」直达会话，完成后自动勾选、动态补位；
- 每个阶段与整体计划均有预计完成日期，进度与剩余天数一目了然。

### 划线摘录笔记

看到关键内容，不用复制粘贴到别处——直接在回答上**划选文本**，选择「摘录为笔记」，内容就变成一张原子笔记卡片。

摘录时可以补充你自己的理解与标签，让笔记不仅是摘抄，更是经过思考的沉淀。

### 划线分支会话

对话中产生新的疑问？对任意一段划线内容选择「分叉」，就能**从这条线索展开一条新的分支会话**，在分支里继续深挖追问，而不打断主线。

所有分支构成一棵会话树，随时回到任意节点继续学习，深度思考一目了然。

### 原子笔记

笔记以「一张卡片一件事」的原子形式组织：

- 含**类型、标签、原文摘录**与**你的理解**；
- 笔记之间可通过 Markdown 双向链接互相引用；
- 全部为 Markdown 文件，存放在 `<Vault>/notes/`，兼容任意 Markdown 工具编辑。

### 间隔复习系统

到期笔记按记忆曲线自动进入复习队列：

- **未来一周复习时间轴**：按天分组展示未来一周的复习安排，逾期归今天并高亮标注，复习节奏心中有数；
- 支持**经典间隔算法**与 **FSRS 个性化算法**两套调度；
- 复习时按笔记内容自动出题，支持**六类题型**（填空、问答、判断等），从不同角度检验记忆；
- 无 API Key 时仍可进入**原文模式**，直接对照笔记原文复习。

### 学习地图与学习总览

- **学习地图**：集中处理到期复习、执行学习计划，复习节奏与今日任务一站搞定；
- **学习总览**（主界面）：累计问答 / 复习 / 笔记数、连续学习天数、学习频率格子图——学了什么、学了多少，一目了然。

### 本地优先的 Vault

知枝的全部数据都在**你自己的本地文件夹（Vault）**里：

| 数据类型 | 位置 |
|----------|------|
| 笔记（Markdown） | `<Vault>/notes/*.md` |
| 学习会话 / 分支 | `<Vault>/sessions/*.md` |
| 参考资料 | `<Vault>/references/` |
| 复习队列、会话树等元数据 | `<Vault>/.study-thread/` |

- 复制 Vault 文件夹 = 完整备份；卸载应用不会删除 Vault；
- 对话内容仅在你选择的 LLM 服务商处处理，知枝本身不收集、不上传任何学习数据。

### 常用快捷键

| 快捷键 | 动作 |
|--------|------|
| `Ctrl + N` | 新建会话 |
| `Ctrl + B` | 打开资料库 |
| `Ctrl + H` | 打开学习地图 |
| `Ctrl + ,` | 打开设置 |

---

## 界面预览

### 学习总览（主界面）

累计学习数据、连续天数与学习频率一目了然。

![主界面展示](docs/screenshots/主界面展示.png)

### 学习会话

与 AI 对话学习，支持划选进行分支追问。（token消耗高，跟正常会话消耗相同，一个汉字约1.5token）

![主会话展示](docs/screenshots/主会话展示.png)

### 划线摘录笔记

在 AI 回答上划选关键内容，一键摘录为原子笔记卡片。可通过设置选择是否由AI生成标题。（token消耗低）

### 链接笔记

在笔记中通过[[]]链接其他笔记，可以点击跳转，便于复习。

![插入笔记展示](docs/screenshots/插入笔记展示.png)

### 划线分支会话

对划线内容分叉展开新分支，在不打断主线的前提下深挖追问。

![分支会话展示](docs/screenshots/分支会话展示.png)

### 资料库（原子笔记）

以卡片形式浏览、检索你的原子笔记。

![笔记界面展示](docs/screenshots/笔记界面展示.png)

### 笔记关系图

笔记之间通过双向链接互相关联，形成知识网络。

![笔记关系图展示](docs/screenshots/笔记关系图展示.png)

### 参考资料

上传 PDF / Markdown / 图片作为参考资料，供对话检索引用。

![参考资料展示](docs/screenshots/参考资料展示.png)

### 间隔复习

到期笔记自动进入复习队列，按记忆曲线间隔重复出题。

![复习界面展示](docs/screenshots/复习界面展示.png)

### 复习答题

由ai自动出题、判断正误，支持填空、问答、判断等多种题型，从不同角度检验记忆。（token消耗中）

![复习界面详细展示](docs/screenshots/复习界面详细展示.png)

---

## 数据与隐私

- **不自动上传**：笔记、会话、计划、参考资料、复习记录均保存在本地 Vault；
- **对话内容**仅在你配置的 LLM 服务商处处理，请留意各家服务的隐私政策；
- **凭据安全**：官方通道的登录凭据与 API Key 只存操作系统钥匙串（Windows 凭据管理器），不落本地明文；可随时退出登录并吊销会话；
- **API Key** 保存在应用本地存储中，请勿在共享设备上使用；
- **备份**：复制 Vault 文件夹即可完整备份全部学习数据。

---

## 快速开始（安装）

1. 前往 [Releases 页面](https://github.com/Altria-aPendrag0n/Zhizhi/releases)，选择最新版本。
2. 在 Assets 中下载对应安装包：
   - **Windows 推荐下载 `.exe`（NSIS 安装包）**，体积更小、安装更快；
   - 也可选择 `.msi`（Windows Installer），适合企业环境统一部署。
3. 双击安装包，按向导完成安装（可选择安装路径）。
4. 从开始菜单或桌面快捷方式启动「知枝」。
5. 首次启动会弹出欢迎引导，按提示完成三步配置（选择 Vault → 配置 AI 服务商 → 开始学习）。

> **SmartScreen 提示**：测试版安装包尚未签名，Windows 可能提示「已保护你的电脑」。点击「更多信息 → 仍要运行」即可。
>
> 详见 [用户手册](docs/user-guide.md)。

---

## 快速开始（开发）

```bash
cd study-thread
npm install
npm run tauri dev
```

安装使用请见 [用户手册](docs/user-guide.md)，环境准备与打包分发请见 [运行与打包指南](docs/RUN.md)。

## 文档

| 文档 | 读者 | 说明 |
|------|------|------|
| [用户手册](docs/user-guide.md) | 最终用户 | 安装、快速上手、核心概念、数据与隐私、FAQ |
| [运行与打包指南](docs/RUN.md) | 开发者/维护者 | 环境准备、开发运行、打包构建、安装包分发 |
| [开发设计文档](docs/DEVELOPMENT.md) | 开发者 | 产品设计、功能规格 |
| [Code Wiki](docs/code-wiki/README.md) | 开发者 | 代码实现角度的模块文档 |
| [发布计划](docs/v0.1-release-plan.md) | 维护者 | v0.1 测试版发布评审与待办存档 |
| [更新日志](CHANGELOG.md) | 所有人 | 版本迭代记录 |

## 仓库结构

```
docs/             # 文档（用户手册 / 运行指南 / 开发文档 / code-wiki / 发布计划）
scripts/          # 开发辅助脚本（窗口恢复、截图等）
study-thread/     # 应用主体（前端 Vue 3 + Rust 后端 Tauri v2）
```

> 官方 API 服务端（账号体系 + API 分发网关 + 综合管理控制台）已迁移至独立私有仓库 **Zhizhi-Server**（`d:\work\Zhizhi-Server` / github.com/Altria-aPendrag0n/Zhizhi-Server），相关 code-wiki 模块（21/23/24）随迁。

## 支持作者

项目已入驻[爱发电](https://ifdian.net/a/asahiiiasd)，如果你觉得知枝对你有帮助，欢迎前往支持我的工作，你的支持是项目持续更新的动力。

## 许可证

[Apache-2.0](LICENSE) © 2026 Asahiiiasd。允许自由使用、修改与分发（含商用），需保留版权声明与许可证文本。

> 说明：本项目为 AI 辅助生成代码，借鉴了 Obsidian（Vault + Markdown 双向链接）与对话式智能导师等产品的**设计理念**，源码为原创。若后续发现混入受 GPL 约束的第三方代码片段，将单独替换并在此说明。
