...
>
> 修复记录 2（2026-09-02，布局错乱回归）：嵌套路由上线后子页面布局错乱（标题与内容横向排列）。根因是 Vue scoped CSS 规则——**子组件根元素会同时命中父组件的 scoped 样式**：三个子页面根元素类名与布局根类名同为 `settings-page`，被布局的 `.settings-page { display: flex; height: 100% }` 误命中。修复：子页面根类名改为语义化命名（`model-config-page` / `official-model-page` / `custom-model-page`），与布局类名解耦。教训：嵌套路由布局中，布局根类名不要与子页面根类名同名。
...
- 官方 API：`settings.getProviderConfig()` 在 `officialApiEnabled`（登录后由 auth store 置位）时返回官方配置（`openai-compat` + 官方 baseUrl + 内存 api\_key），LLM 调用链路不变。
