...
- 维护"项目 → 会话"两级导航模型；**会话列表来自 vault `sessions/*.md`**（仓库即真相，无 localStorage 缓存）。
...
```ts
projects: Project[]                          // { id, name }，固定 3 个项目
activeProjectId / activeThreadId
breadcrumbs                                  // ['学习会话', 会话标题]
threads = computed(sessionStore.sessionList) // 来自 vault sessions/*.md，无本地缓存
```
