...
`hideThreads` 时通过 `app-shell--threads-hidden` 类将 Grid 变为 3 列；会话列用 `<Transition name="threads-collapse">` 淡出/移入，配合 `grid-template-columns` 过渡，避免 `hideThreads` 切换时布局跳变闪烁（如从聊天页进入资料库页）。
