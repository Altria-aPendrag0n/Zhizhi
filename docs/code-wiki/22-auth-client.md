...
服务端 Phase 1（验证码登录 / access+refresh / /api/me）就绪后，客户端接入真实登录流。核心目标是**凭据安全边界**：官方 Key（= 钱）与 refresh_token（= 会话永续）绝不进 localStorage，只存 OS 钥匙串；access_token 仅内存，靠轮换静默续期。同时解决开发期本地服务与发布期生产地址的配置问题。
...
- `sendCode(identifier)` → 返回 `cooldown_seconds`（UI 倒计时）。
- `login(identifier, code)` → zhizhi-api 登录 → 写钥匙串（refresh_token + 首登 api_key）→ 内存 accessToken/apiKey → `settings.officialApiEnabled = true` → `fetchMe()` 拉额度。
...
- `sendCode / login / logout / fetchMe / refreshSession`：对应服务端端点。
...
## OfficialModelPage 登录流

- anonymous：账号 + 验证码表单；「获取验证码」60s 倒计时（`cooldown_seconds`）；错误 toast（401 验证码错误 / 429 限频 / 网络）。
- authenticating：按钮 loading。
- authenticated：展示账号 / 套餐 / 剩余额度（`/api/me`）+ 「退出登录」/「完成」。
...
| 文件 | 覆盖 |
|---|---|
| `src/api/zhizhi-api.test.ts` | login 请求与解析、不安全 baseUrl 拒绝（不发请求）、网络错误 NETWORK、429 冷却秒数、**并发 401 只刷新一次并重放**、刷新失败清凭据抛 SESSION_EXPIRED、refreshSession 成功/无凭据 |
| `src/stores/auth.test.ts` | sendCode 冷却、login 成功（钥匙串写入+启用官方）、login 失败回匿名、restore 四分支（无凭据/刷新成功/刷新失败/缺 Key）、logout 清凭据、fetchMe 额度同步 |
| `src/views/OfficialModelPage.test.ts` | 表单渲染、空账号提示、发码倒计时（fake timers）、登录成功进已登录态、登录失败保持表单、登出回表单、套餐未上线提示、返回跳转 |
