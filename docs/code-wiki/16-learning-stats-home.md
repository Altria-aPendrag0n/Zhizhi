...
- GitHub 风格 53 周（近一年）× 7 天，今天所在周居右；周一在列顶、周日列底；`row = (getDay() + 6) % 7`。
- **尺寸自适应**：格子边长由容器宽度均分得出（`--cg-cell` 随测量宽度动态更新，`ResizeObserver` 监听），尽量填满容器宽度；上限 `28px` 保证纵向紧凑，未测量到宽度时兜底 10px。图表整体按内容宽度居中，留白均匀。
- 每天一个格子，颜色分 5 档（`--cg-empty` + `--cg-l1..l4` 绿色递增）。
- 颜色按**固定阈值**分层（淡绿到浓绿间隔拉大）：0 次无色；1-2 次 lvl1；3-4 次 lvl2；5-9 次 lvl3；10+ 次 lvl4（浓绿）。
