...
- **复习问答页（ReviewChatPage）**：进度显示为品牌胶囊；四档自评按钮（忘了/模糊/记得/熟练）各带底色 + 提示文案（不依赖 hover 才区分）；结构化答题区（选择/对错/填空/排序）置于卡片容器。**题目直接展示在会话流中**（`appendQuestionMessage` 将当前题目作为 assistant 消息注入，含题号「第 X 题 / 共 N 题」+ 题型标签 + 题干，选择题附带选项、排序题附带乱序步骤），随会话一起滚动与持久化、恢复会话不重复注入；**无独立问题框/拖拽手柄**。下方保留结构化答题卡片与 Composer；顶部「复习会话 + 笔记标题」弱化为小字淡色信息条；对话区为独立滚动容器（`__body` flex 容器 + `ChatView` 内部 `overflow-y: auto`），历史会话内容、已答反馈与题目均可滚动查看。
