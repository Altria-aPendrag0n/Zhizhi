...
- **复习问答页（ReviewChatPage）**：进度显示为品牌胶囊；四档自评按钮（忘了/模糊/记得/熟练）各带底色 + 提示文案（不依赖 hover 才区分）；结构化答题区（选择/对错/填空/排序）置于卡片容器。**题目框成为答题区主角**——左右默认占满、**可上下自由拉伸**（`resize: vertical`，高度限制 `min-height 108px / max-height 55vh`），题号与题型胶囊固定于顶部，**题干区域独立滚动**（长题干显示不全时内部滚动查看）；顶部「复习会话 + 笔记标题」弱化为小字淡色信息条；对话区为独立滚动容器（`__body` flex 容器 + `ChatView` 内部 `overflow-y: auto`），历史会话内容与已答反馈可滚动查看，当前题目固定在底部。
