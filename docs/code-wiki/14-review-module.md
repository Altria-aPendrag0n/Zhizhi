...
- **复习问答页（ReviewChatPage）**：进度显示为品牌胶囊；四档自评按钮（忘了/模糊/记得/熟练）各带底色 + 提示文案（不依赖 hover 才区分）；结构化答题区（选择/对错/填空/排序）置于卡片容器。**题目居中放大成为答题区主角**（题号 + 题型胶囊 + 大号题干居中展示），顶部「复习会话 + 笔记标题」弱化为小字淡色信息条，视觉重心放在当前题目与作答区；对话区为独立滚动容器（`__body` flex 容器 + `ChatView` 内部 `overflow-y: auto`），历史会话内容与已答反馈可滚动查看，当前题目固定在底部。
