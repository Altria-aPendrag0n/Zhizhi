...
- **复习页定位**：`/review/:id` 为独立复习会话页，顶栏面包屑显示「学习地图 / 复习」（`App.vue` 显式处理 `/review` 路由，不再回落默认"学习会话"面包屑），左侧项目栏高亮学习地图，避免与知枝学习会话混淆。
- **视图深链接**：学习地图视图由 `?view=review|network` 驱动（`switchView` 切换时同步 URL query）；主界面「待复习」点击直达复习视图 `/hub?view=review`。学习总览视图已移除（与主界面数据总览重合），复习为默认首位视图，认知地图（原概念网络）标注开发中。
- **到期清单（ReviewDueList）**：卡片左侧按笔记类型显示色条（概念绿/方法蓝青/事实琥珀/问题紫），衬线标题 + 品牌绿类型胶囊；「开始复习」主按钮品牌绿填充带阴影；四档评级按钮 hover 分别呈现 红/琥珀/绿/深绿；掌握度进度条品牌绿渐变；空态带品牌软色装饰图标。
- **复习问答页（ReviewChatPage）**：进度显示为品牌胶囊；四档自评按钮（忘了/模糊/记得/熟练）各带底色 + 提示文案（不依赖 hover 才区分）；结构化答题区（选择/对错/填空/排序）置于卡片容器。**题目框成为答题区主角**——左右默认占满、**顶部自定义拖拽手柄可上下自由拉伸**（替代 CSS `resize: vertical`，因 WebView2 中该属性不可靠；通过 `mousedown`/`mousemove`/`mouseup` 事件实现，**向上拖拉高、向下拖拉低**，高度限制 `min-height 108px / max-height 55vh`），手柄 hover/dragging 时变品牌色提供视觉反馈；题号与题型胶囊固定于手柄下方，**题干区域独立滚动**（长题干显示不全时内部滚动查看）；顶部「复习会话 + 笔记标题」弱化为小字淡色信息条；对话区为独立滚动容器（`__body` flex 容器 + `ChatView` 内部 `overflow-y: auto`），历史会话内容与已答反馈可滚动查看，当前题目固定在底部。
- **答题组件（Choice/TrueFalse/FillBlank/OrderingAnswer）**：统一接入品牌令牌（`--line`/`--brand`/`--brand-soft`/`--r-*`），替换原先的默认 `--border`/`--color-primary` 兜底色；对/错题重做为两个大卡片按钮，带 ✓/✕ 图标与绿/红语义色。
- **AI 等待提示**：流式生成中（`isStreaming`）在输入区上方显示品牌软色提示条「AI 正在思考…请勿重复提交」+ 旋转 spinner；输入框与结构化答题同时 `disabled`，防止重复提交。
